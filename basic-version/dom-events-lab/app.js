...
function handleButtonPress(e) {
    ...
    switch (value) {
        ...
        default: {operation[1] ? operation[1].push(value) : operation[0].push(value); render('number'); console.log(operation)};
        };
        // Clear state after render result. Render only the current operation the first time 
        if (/^[\+-\/\*]$/.test(value)) {
            console.log("operator:",value);
            if (operation[1]) {reset();
            } else {render('operator'); operation.push([]);console.log(operation)};
        };
};


// FUNCTIONS
function num(val) {console.log("Value:",val); return Array.isArray(val) ? Number(val.join('')) : Number(val)};
